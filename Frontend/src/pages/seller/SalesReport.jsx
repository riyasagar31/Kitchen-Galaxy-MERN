...
import { FiArrowLeft, FiFilter, FiCalendar, FiDownload } from 'react-icons/fi';
        ...
        <div className="max-w-7xl mx-auto px-4 py-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
                <div className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm">
                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Total Sales (Items)</p>
                    <div className="text-3xl font-bold text-gray-900">₹{summary.totalSales.toLocaleString()}</div>
                ...
                <div className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm">
                    ...
                    <div className="text-3xl font-bold text-[#ff5252]">₹{summary.gstCollected.toLocaleString()}</div>
                    <p className="text-[10px] text-gray-400 font-medium mt-1">Pending payout logic</p>
                </div>
            </div>
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-100">
                        <thead className="bg-[#f8f9fa]">
                            <tr>
                                <th className="px-8 py-5 text-left text-[10px] font-bold text-gray-400 uppercase tracking-widest">Order Details</th>
                                <th className="px-8 py-5 text-left text-[10px] font-bold text-gray-400 uppercase tracking-widest">Qty</th>
                                <th className="px-8 py-5 text-left text-[10px] font-bold text-gray-400 uppercase tracking-widest">Price / GST</th>
                                <th className="px-8 py-5 text-left text-[10px] font-bold text-gray-400 uppercase tracking-widest">Total Amount</th>
                                <th className="px-8 py-5 text-left text-[10px] font-bold text-gray-400 uppercase tracking-widest">Status</th>
                            </tr>
                        </thead>
                        <tbody className="bg-white divide-y divide-gray-100">
                            {loading ? (
                                <tr><td colSpan="5" className="px-8 py-20 text-center text-gray-400 italic font-medium">Loading report data...</td></tr>
                            ) : reportData.length === 0 ? (
                                <tr><td colSpan="5" className="px-8 py-20 text-center text-gray-400 italic font-medium">No sales recorded for this period.</td></tr>
                            ) : reportData.map((row, idx) => (
                                <tr key={idx} className="hover:bg-gray-50/50 transition-colors">
                                    <td className="px-8 py-6">
                                        <div className="text-sm font-bold text-gray-900">{row.productName}</div>
                                        <div className="flex items-center gap-2 mt-1">
                                            <span className="text-[10px] font-black text-gray-400 uppercase tracking-tight">#{row.orderId.slice(-6).toUpperCase()}</span>
                                            <span className="text-[10px] text-gray-300">•</span>
                                            <span className="text-[10px] font-bold text-gray-400">{new Date(row.orderDate).toLocaleDateString()}</span>
                                        </div>
                                    </td>
                                    <td className="px-8 py-6">
                                        <span className="text-sm font-bold text-gray-700">x {row.qty}</span>
                                    </td>
                                    <td className="px-8 py-6">
                                        <div className="text-sm font-bold text-gray-900">₹{row.basePrice.toLocaleString()}</div>
                                        <div className="text-[10px] font-bold text-gray-400 mt-0.5">GST ({row.gstRate}%): ₹{row.gstAmount.toLocaleString()}</div>
                                    </td>
                                    <td className="px-8 py-6">
                                        <div className="text-base font-black text-[#ff5252]">₹{row.totalAmount.toLocaleString()}</div>
                                    </td>
                                    <td className="px-8 py-6">
                                        <span className={`px-3 py-1 text-[9px] font-black rounded-full uppercase tracking-tight ${row.status === 'Delivered' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                                            {row.status}
                                        </span>
                                    </td>
                                </tr>

...
export default function AdminCategories() {
  ...
  return (
    <div className="bg-white shadow-sm rounded-xl p-6 border border-gray-100">
      ...
      {/* Edit Overlay (Subtle) */}
      {expandedEditId && (
        <div className="mt-4 p-5 bg-white border-2 rounded-xl shadow-xl animate-in zoom-in-95 duration-200" style={{ borderColor: THEME_COLOR }}>
          <h4 className="text-sm font-black mb-3 uppercase tracking-tighter" style={{ color: THEME_COLOR }}>Edit Category Name</h4>
          <div className="flex gap-2">
            <input 
              className="flex-1 border border-gray-300 p-2.5 rounded-lg focus:outline-none focus:ring-2 shadow-sm" 
              style={{ '--tw-ring-color': THEME_COLOR }}
              value={editForm.name} 
            ...
            <button 
              style={{ backgroundColor: THEME_COLOR }}
              className="text-white px-6 py-2 rounded-lg text-sm font-bold shadow-lg shadow-red-100 hover:brightness-110" 
              onClick={() => saveEdit(expandedEditId)}
            >
              Update
            </button>
        ...
        <div className="mt-6 p-6 rounded-xl border-l-4 shadow-sm animate-in slide-in-from-left-2 duration-300" 
          ...
          <div className="flex justify-between items-start">
            <div>
              ...
              <h4 className="text-2xl font-bold mt-1" style={{ color: THEME_COLOR }}>{viewedCategory.name}</h4>
              <p className="text-sm text-gray-500 mt-2">
                Created on: {new Date(viewedCategory.createdAt).toLocaleDateString()}
              </p>
            </div>
            <button 
